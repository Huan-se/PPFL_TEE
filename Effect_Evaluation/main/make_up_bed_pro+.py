...
def smooth_curve_interpolation(scalars, kernel_size=7):
    """
    双重平滑处理：中值滤波去刺 + 三次样条插值圆润
    kernel_size: 中值滤波的窗口大小，必须是奇数。
                 如果是 3，可以滤除单点尖刺；如果是 5，可以滤除连续两点的尖刺。
    """
    ...
    # ==========================================
    # 第一步：中值滤波 (Median Filter) 削平突变尖刺
    # ==========================================
    # 它会将窗口内的极大值/极小值作为异常点抛弃，保留中位数，保持趋势不变
    y_despiked = medfilt(y, kernel_size=kernel_size)
    
    # ==========================================
    # 第二步：三次样条插值 (Cubic Spline) 视觉圆润化
    # ==========================================
    x_smooth = np.linspace(x.min(), x.max(), 300)
    try:
        spline = make_interp_spline(x, y_despiked, k=3)
